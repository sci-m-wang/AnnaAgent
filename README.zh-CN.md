...
`config wizard` 和 `config secrets` 会用隐藏密码输入模式读取 API key，并写入 `.env`。初始化时生成的 `.env` 和 `.env.example` 已包含 AnnaAgent backbone、SFT model、embedding 等 key 的注释占位符；如果你想手动编辑，直接在对应占位符下面取消注释并填写即可。API key 等敏感信息不要提交到 Git。
...
下载资产时请始终传入 `--workspace` 或 `--manifest`。如果两个都不传，`anna assets download` 会把当前目录当成工作区，可能下载到 `./assets/...`，而不是你的 AnnaAgent 工作区。你可以只下载某一个资源，也可以显式覆盖目标目录：
...
```bash
anna assets download paper --workspace anna-workspace
anna models env setup --workspace anna-workspace  # 如果 create 时已用 --deploy-env，可跳过
anna models env status --workspace anna-workspace
...
如果没有传 `--model-path`，`models deploy` 会从 `assets/anna-assets.json` 中读取对应 SFT 资源的 target 路径，包括你在 JSON 中写的绝对路径。请确保 deploy 时使用和 download 时相同的 `--workspace` 或 `--manifest`。
