...
.. code-block:: bash
   ...
   anna init from-prompt anna-workspace/prompts/full.json
   anna chat --workspace anna-workspace --state anna-workspace/prompts/full.json

``full`` is the only command that generates a reusable prompt state. It runs the
complete AnnaAgent initialization pipeline and may call multiple model services.
``from-prompt`` validates a prebuilt state generated by ``full``.
